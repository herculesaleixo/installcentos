...
read -p "Please insert the Public DNS or Public IP of instance: " DOMAIN

export DOMAIN=${DOMAIN:="$(curl ipinfo.io/ip)"}
export USERNAME=${USERNAME:=root}
export PASSWORD=${PASSWORD:=Toor#123}
export VERSION=${VERSION:="v3.7.0"}

export SCRIPT_REPO=${SCRIPT_REPO:="https://raw.githubusercontent.com/herculesaleixo/installcentos/master"}
...
yum install -y git wget nano net-tools docker ansible \
python-cryptography pyOpenSSL.x86_64 \
...
echo "******"
echo "* Your console is https://$DOMAIN:8443"
echo "* Your username is $USERNAME "
echo "* Your password is $PASSWORD "
echo "*"
echo "* Login using:"
echo "*"
echo "$ oc login -u ${USERNAME} -p ${PASSWORD} https://$DOMAIN:8443/"
echo "******"

oc login -u ${USERNAME} -p ${PASSWORD} https://$DOMAIN:8443/
